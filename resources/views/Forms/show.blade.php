            ...
            <div class="mb-4">
                ...
                @if (in_array($field['type'], ['text', 'email', 'date', 'tel']))
                    <input type="{{ $field['type'] }}" name="{{ $name }}" id="{{ $name }}"
                        value="{{ old($name) }}" class="w-full border rounded px-3 py-2"
                        placeholder="{{ $field['placeholder'] ?? '' }}"
                        {{ !empty($field['required']) ? 'required' : '' }}>
                @endif

                @if ($field['type'] === 'textarea')
                    <textarea name="{{ $name }}" id="{{ $name }}" class="w-full border rounded px-3 py-2"
            ...
            {{ old($name) }}
                ...
                @if ($field['type'] === 'select')
                    <select name="{{ $name }}" id="{{ $name }}"
                        class="w-full border rounded px-3 py-2" {{ !empty($field['required']) ? 'required' : '' }}>
                        ...
                        @foreach ($field['options'] as $option)
                            <label>
                                <input type="{{ $field['type'] }}" name="{{ $name }}"
                                    value="{{ $option }}" {{ !empty($field['required']) ? 'required' : '' }}>
                                {{ $option }}
                            </label>
                        @endforeach
                    </div>
                ...
                @if ($field['type'] === 'file' || $field['type'] === 'url')
                    <input type="{{ $field['type'] }}" name="{{ $name }}" id="{{ $name }}"
                        class="w-full border rounded px-3 py-2" placeholder="{{ $field['placeholder'] ?? '' }}"
                        {{ !empty($field['required']) ? 'required' : '' }}>
                @endif
                @error($name)
